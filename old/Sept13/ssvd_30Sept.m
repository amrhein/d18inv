...
% load mkDlsfile%q [Dwd, Dwdt,tp, yil, dtlims, core_order, bv, Pm, d18m, d18mP]
...
% column weight by average fluxes
...
%Gbp = bsxfun(@times,Gbp,repmat(1./(Vtot'/nansum(Vtot(:))),1,101));
Gbp = Gbptv;
% 
% save Gbp Gbp
...
load('4weddell/Gbpusv30Sept_V.mat');
vs = v;
...
[xests] = tsvd(u,s,vs,yp,k);
yest = Gbp*xests;
yestu = cPm*yest + xic10k2h(~bv);

xest = zeros(2806*101,1);
xest(tpr) = xests;
