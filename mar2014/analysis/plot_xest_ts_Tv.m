...
%sb = indmap(~isnan(indmap) & latmg<-60 & ((lonmg>=0 & lonmg<=30)));

% similar area to the nb
%sb = indmap(~isnan(indmap) & (latmg<-50 ) & (lonmg>=300 & lonmg<=360));
...
export_fig('-eps',['Figs/xest_map',num2str(k)])
...
set(get(h,'xlabel'),'String', 'Resolution');
...
xlabel('Time (years)')
ylabel('Inferred ML \delta^1^8O_c (permil)')
hold on
